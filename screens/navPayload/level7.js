...
const Level7 = ({navigation, route}) => {
  ...
  console.log('complite7Lvl==>', complite7Lvl);
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`Level7`, jsonData);
  ...
  const handleOptionSelect = index => {
    ...
    if (
      ...
      questions7[currentQuestionIndex].correct_order.length
    ) {
      if (
        JSON.stringify(newSelectedOptions) ===
        JSON.stringify(questions7[currentQuestionIndex].correct_order)
      ) {
      ...
      if (currentQuestionIndex < questions7.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
        setSelectedOptions([]);
      } else {
        if (correctAnswers + 1 === 10) {
          setComplite7Lvl(true);
      ...
      <View style={{flex: 1}}>
        <ScrollView>
          <View style={styles.contentConteiner}>
            ...
            {questions7[currentQuestionIndex] && (
              <>
                ...
                {questions7[currentQuestionIndex].options.map(
                  (option, index) => (
                    <TouchableOpacity
                      key={index}
                      style={[
